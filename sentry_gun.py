...
while True:
    ...
    # Calculamos el peso acumulado del fondo, para abstraer el movimiento
    cv2.accumulateWeighted(gray, background_frame, 0.5)

    #  Calculamos la diferencia absoluta entre el primer frame
    #  y el frame actual (Es decir, el frame delta)
    frameDelta = cv2.absdiff(gray, cv2.convertScaleAbs(background_frame))


    # Umbralizamos el frameDelta obtenido anteriormente, para facilitar la detección
